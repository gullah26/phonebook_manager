...
def add_contact():           
    ...
    save = input("Confirm and save y/n to quit: ")
    if save == "y":
        print("Contact saved")
...
def search_contact():
    ...
    contact_search = input("Search directory: ")
    with open("directory.txt") as directory_file:
        phone_directory = directory_file.readlines()
        # for contact_search in phone_directory:
        #     print(contact_search)
        found = False
        for line in phone_directory:
            for word in line.split():
                if contact_search in word:
                    print(contact_search)
                    found = True
                    break
    if not found:
    ...
    line_index = 0
    for contact in phone_directory:
        if contact_search in contact:
            new_content.insert(line_index, contact)
            line_index += 1
    directory_file.close() 
    if len(new_content) == 0:
        print(f"\n{contact_search} is not found in phonebook!")
    else:
        contactLen = len(new_content)
        print("@" * 40)
        print(f"\n**** Lines containing {contact_search}****\n")
        print("@" * 40)
